...
fit_model(y::Vector{Fl};
            ...
            model_functions::Dict = Dict("create_X" => create_X_unobserved_components, "get_components_indexes" => get_components_indexes,
                                        "get_variances" => get_variances, "forecast" => forecast_unobserved_components,
                                        "model_parameters" => ["stochastic_level", "trend", "stochastic_trend", "seasonal", "stochastic_seasonal", "freq_seasonal"]),
    ...
    - model_functions::Dict: Dictionary containing the model functions (default: Dict("create_X" => create_X_unobserved_components, "get_components_indexes" => get_components_indexes, "get_variances" => get_variances, "forecast" => forecast_unobserved_components)).
...
function fit_model(y::Vector{Fl};
    ...
    @assert all([key in keys(model_input) for key in model_functions["model_parameters"]]) "All model parameters must be set to true in model_input"

    X = model_functions["create_X"](model_input, Exogenous_X, outlier, ζ_ω_threshold, T)

    estimation_y, Estimation_X, valid_indexes = handle_missing_values(X, y)

    components_indexes  = model_functions["get_components_indexes"](T, Exogenous_X, model_input, outlier, ζ_ω_threshold)
    ...
    return Output(model_input, model_functions, X, coefs, ε, fitted, components, residuals_variances, T, outlier, valid_indexes, ζ_ω_threshold, y)
end
...
function forecast(output::Output, steps_ahead::Int64; Exogenous_Forecast::Matrix{Fl}=zeros(steps_ahead, 0))::Vector{Float64} where Fl
    ...
    return output.model_functions["forecast"](output, steps_ahead, Exogenous_Forecast)
end
