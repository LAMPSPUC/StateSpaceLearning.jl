...
const UNOBSERVED_COMPONENTS_PARAMETERS = ["stochastic_level", "trend", "stochastic_trend", "seasonal", "stochastic_seasonal", "freq_seasonal"]
...
fit_model(y::Vector{Fl};
    ...
    - model_functions::Dict: Dictionary containing the model functions (default: Dict("create_X" => create_X_unobserved_components, "get_components_indexes" => get_components_indexes, "get_variances" => get_variances)).
...
function fit_model(y::Vector{Fl};
    ...
    if model_functions["create_X"] == create_X_unobserved_components
        @assert T > model_input["freq_seasonal"] "Time series must be longer than the seasonal period"
        @assert all([key in keys(model_input) for key in UNOBSERVED_COMPONENTS_PARAMETERS]) "The unobserved components model must have all the necessary parameters $(UNOBSERVED_COMPONENTS_PARAMETERS)"
    end
