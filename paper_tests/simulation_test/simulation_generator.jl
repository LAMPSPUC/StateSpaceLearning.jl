...
function generate_sarima_exog(T::Int64, M::Int64)
    ...
    for j in 1:M
        ...
        try_again = true
        ar_params = nothing; ma_params = nothing; sar_params = nothing; sma_params = nothing
        p = nothing; q = nothing; P = nothing; Q = nothing

        while try_again
            p=rand([0, 1,2,3]);q=rand([0, 1,2,3]);P=rand([0,1]);Q=rand([0,1])
        ...
        data = vcat(rand(Normal(0, 1), max(p, q, P * s, Q * s)), zeros(T - max(p, q, P * s, Q * s)))
        for i = max(p, q, P * s, Q * s) + 1:T
            seasonal_comp = 0
            if i > s
                P_term = P == 0 ? 0 : sum(sar_params .* data[(i - s * P):(i - s):(i - s * P + 1)])
                Q_term = Q == 0 ? 0 : sum(sma_params .* data[(i - s * Q):(i - s):(i - s * Q + 1)])
